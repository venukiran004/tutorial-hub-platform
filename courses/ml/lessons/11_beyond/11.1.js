...
# rollback = point the service at the previous artefact, re-run the golden test (0.0e+00 on its own stored predictions), AND restore its threshold`,
      caption: "A registry is a table of versions with their code, data hash, environment, metrics and stage (staging, production, archived); a model card is the human-readable half — what it is for, what it was trained on, where it fails. The rollback shows why the threshold and the calibration belong in the artefact: two versions with identical AUC disagreed on half the decisions, because one was class-weighted and the other calibrated. Rolling back the model without rolling back the threshold would have sextupled the retention queue." },

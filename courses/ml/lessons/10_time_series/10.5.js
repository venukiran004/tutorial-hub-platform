...
the same recursion on a FITTED model (a simulated ARIMA(1,1,1)+drift, n = 600):
  ...
  statsmodels forecast: 910.997  912.294  913.701  915.162`,
    ...
    { t: "callout", kind: "trap", title: "Twelve points cannot estimate an ARIMA(1,1,1)", body: "Fitted to the twelve quarterly numbers alone, statsmodels returns φ = −1.000 and θ = 0.996 — the boundary of the parameter space, where the AR root sits on the unit circle and the MA term tries to cancel it. That is not a model; it is an optimiser with nothing to work with. The hand example above uses *given* parameters for that reason. ARIMA needs enough history to identify its orders and estimate its parameters — a few dozen points for a simple model, several seasons for a seasonal one — and the exercise in 10.4 and the store fits below have 900." },

...
CRPS of a POINT forecast (every path collapsed to the median): 20.53 = its MAE`,
    ...
    { t: "callout", kind: "production", title: "What the interval is for", body: "An 80 % interval that covers 87 % is not 'better' — it is wider than it needs to be, and a planner holding stock at its upper edge pays for the excess. An interval that covers 44 % is a promise that will be broken one day in two. The deliverable is a band whose coverage matches its label on the recent past, checked every week, with the width reported per horizon so that a 28-day order and a 1-day order are sized differently. Report the loss (pinball or CRPS) for choosing between methods, the coverage for trusting the one chosen, and the width for the decision it feeds." },

...
#  mean                               42.40  +13.98   36.18   43.59   34.89      10.54`,
    ...
    { t: "callout", kind: "mental", title: "The baseline is the denominator", body: "Every metric that compares across series (MASE, §05) divides by the in-sample error of a naive or seasonal-naive forecast, and every forecast-value-added review (10.9) asks how much each step of the pipeline improved on it. Compute the baselines first, on the same backtest, at the same horizon, and keep the number: a model with a 28-day MAE of 24 is a 10 % improvement on 26.9 at store A, which may or may not be worth its cost, and that is the only frame in which the 24 means anything." },
...
Spearman between the two rankings across the 24 configs: 0.78`,
    ...
    { t: "callout", kind: "insight", title: "The one question", body: "For every feature column: at 09:00 on the day this forecast must be produced, would this exact number have been available to me? If the answer is no, or 'probably by then', it is a leak. Encode the answer as an assertion in the pipeline — `assert X_train.index.max() < X_test.index.min()`, `assert lag >= latency` — not as a comment. Every one of the twelve leaks has shipped somewhere, and every one produces the same symptom: a wonderful backtest and a model that adds no value." },
